...
We berkenen de (partitiele) autocorrelaties voor de cummulatieve personeelsverliezen(niveau):
```{r}
max_lag <- 15
...
```
In bovenstaande figuur zijn de voorspellingen in het rood, de werkelijke waardes in het zwart en de betrouwbaarheidsintervallen geplot. In de grafiek kunnen we, conform onze verwachtingen, aan het begin 
...
upper <- fitted(arima_forecast_final_1) + 1.96*sqrt(arima_forecast_final_1$sigma2)
lower <- fitted(arima_forecast_final_1) - 1.96*sqrt(arima_forecast_final_1$sigma2)
plot(russia_losses_personnel$personnel, type="n", ylim=range(lower,upper))
polygon(c(time(russia_losses_personnel$date),rev(time(russia_losses_personnel$date))), c(upper,rev(lower)), col=rgb(0,0,0.6,0.2), border=FALSE)
lines(russia_losses_personnel$personnel)
lines(fitted(arima_forecast_final_1),col='red')
out <- (russia_losses_personnel$personnel < lower | russia_losses_personnel$personnel > upper)
