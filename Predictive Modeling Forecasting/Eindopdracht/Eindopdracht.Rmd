...
Laten we ook een simpel autoregressief model beschouwen met 1 lag. 
```{r}
...
```{r}
actuals_predict_2 <- tibble(
  actual = estimation_sample$personnel[2:nrow(estimation_sample)],
...
AR(1)
```{r}
ar_forecast <- predict(ar_1, n.ahead = 77)
summary(ar_forecast)
arima_forecast_3 <- forecast::Arima(y = test_sample$personnel, order = c(1, 0, 0))
